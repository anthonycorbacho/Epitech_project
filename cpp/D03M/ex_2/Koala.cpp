...
// Last update Wed May  5 17:59:47 2010 anthony1 corbacho
//
...
Koala*		Koala::operator+(Koala& mate)
{
  return (this->procreate(mate));
}
...
void		Koala::operator<<(std::string ingredient)
{
  this->eat(ingredient);
}
Koala&		operator>>(std::string ingredient, Koala& k)
{
  k.eat(ingredient);
  return (k);
}

std::ostream&		operator<<(std::ostream& o, Koala &k)
{
  o << "[" << k.getName()<<"]" <<"Sex = "<< k.getSex()<< std::endl;
  o << "[" << k.getName()<<"]" <<"Name = "<< k.getName()<< std::endl;
  o << "[" << k.getName()<<"]" <<"GenIndex = "<< k.getGenIndex()<< std::endl;
  o << "[" << k.getName()<<"]" <<"Picture ="<< std::endl;
  o <<k.getPicture()<< std::endl;  
  return (o);
}
