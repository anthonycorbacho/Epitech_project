...
// Last update Wed May  5 22:39:10 2010 anthony1 corbacho
//

#include	"ex_1.h"

// Card
Cluedo::Card::Card(Cluedo::Card::Type cardType)
{
  _cardType = cardType;
}
...
Cluedo::Card::Type	Cluedo::Card::CardType() const
{
...
Cluedo::Weapon::Weapon(Weapon::Name name, bool bearsFingerprints)
  :Cluedo::Card(Cluedo::Card::WEAPON)
{
...
Cluedo::Suspect::Suspect(Suspect::Name name, bool isLying)
  :Cluedo::Card(Cluedo::Card::SUSPECT)
{
...
Cluedo::Room::Room(Cluedo::Room::Name name, bool hasSecretPassage)
  :Cluedo::Card(Cluedo::Card::ROOM)
{
  if (Cluedo::Room::PartOfTheCrime ==
      name)
    _isPartOfTheCrime = 1;
  else
    _isPartOfTheCrime = 0;
...
bool	Cluedo::Game::CaseSolved(Card* c[])
{
  bool	bweap = 0;
  bool	bsus = 0;
  bool	broom = 0;

  for (int i = 0; c[i] && !(bweap && bsus && broom); i++)
    {
      if (c[i]->IsPartOfTheCrime())
	{
	  if (c[i]->CardType() == Cluedo::Card::WEAPON)
	    bweap = 1;
	  if (c[i]->CardType() == Cluedo::Card::SUSPECT)
	    bsus = 1;
	  if (c[i]->CardType() == Cluedo::Card::ROOM)
	    broom = 1;
	}
    }
  return (bweap && bsus && bsus);
}
