...
** Last update Tue Apr 20 16:37:26 2010 pierre1 rolland
*/
...
void		put_menu_monsters_sprite(SDL_Rect *sdl)
{
  (*sdl).x = 384;
  (*sdl).y = 0;
  (*sdl).w = 476;
...
void		update_menu_monsters(SDL_Rect *rec)
{
  (*rec).x = 384;
  (*rec).y = 0;
  (*rec).w = 476;
...
void		put_menu_monsters_sdl(t_sdl *sdl)
{
  put_menu_monsters_sprite(&sdl->menu.pos_menu);
  update_menu_monsters(&sdl->menu.rect_menu);
